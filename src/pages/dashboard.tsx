...
import { getGamePlayers } from 'apiSdk/game-players';
...
function DashboardPage() {
  ...
  const gamePlayerStat = useSWR<PaginatedInterface<GamePlayerInterface>>(
    () => `/game-players`,
    () => getGamePlayers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Game Players',
      result: gamePlayerStat?.data?.totalCount,
      isLoading: gamePlayerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
